...
```sh
sudo apt-get install mpg123 libmpg123-dev libasound2
```
- `mpg123` — audio output plugin `.so` files (ALSA, OSS, etc.) needed by `libout123` at runtime
- `libmpg123-dev` — headers for compiling
- `libasound2` — ALSA runtime library required by the `output_alsa.so` plugin

> On Raspberry Pi OS `libasound2` is usually already installed. Verify with:
> ```sh
...
### Host (Fedora/RHEL)
```sh
sudo dnf install mpg123 mpg123-devel alsa-lib
```
...
```
Via `make menuconfig` → Target packages → Libraries → Audio/Sound,
