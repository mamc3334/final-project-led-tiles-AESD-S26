...
struct mp3_player {
    ...
    char            path[4096];
    char            device[256];     /* ALSA device, e.g. "hw:2,0"       */
};
...
static void *playback_thread(void *arg)
{
     ...
     * Use the configured device string if set, otherwise fall back to default. */
    const char *device = p->device[0] ? p->device : NULL;
    const char *drivers[] = { "alsa", "oss", "pulse", NULL };
    int opened = 0;
    for (int i = 0; drivers[i] != NULL; i++) {
        if (out123_open(ao, drivers[i], device) == OUT123_OK) {
            printf("[mp3_player] Using audio driver: %s, device: %s\n",
                   drivers[i], device ? device : "default");
            opened = 1;
            break;
        }
    }
...
mp3_player_t *mp3_player_create(void)
{
    ...
    p->device[0] = '\0';  /* empty = use default */
    return p;
}
